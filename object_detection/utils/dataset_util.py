...
def read_dataset(file_read_func, decode_func, input_files, config):
  ...
  try:
    records_dataset = filename_dataset.apply(
			tf.contrib.data.parallel_interleave(file_read_func, cycle_length=config.num_readers, block_length=config.read_block_length, sloppy=True))
  except:
    # For tf < 1.5
    records_dataset = filename_dataset.apply(
			tf.contrib.data.sloppy_interleave(map_func=file_read_func,cycle_length=config.num_readers,block_length=config.read_block_length))


  if config.shuffle:
    records_dataset.shuffle(config.shuffle_buffer_size)
  tensor_dataset = records_dataset.map( decode_func, num_parallel_calls=config.num_parallel_map_calls)
