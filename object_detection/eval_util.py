    ...
    try:
      for batch in range(int(num_batches)):
        if batch % 500 == 0:
          ...
          print('-> Running eval ops batch {}/{}'.format(batch + 1, num_batches))
        ...
        for evaluator in evaluators:
          evaluator.add_single_ground_truth_image_info(image_id=batch, groundtruth_dict=result_dict)
          evaluator.add_single_detected_image_info(image_id=batch, detections_dict=result_dict)
...
def repeated_checkpoint_run(tensor_dict,
  ...
  while True:
    start = time.time()
    print('-> Starting evaluation at {}'.format(time.strftime('%Y-%m-%d-%H:%M:%S', time.gmtime())))
    model_path = tf.train.latest_checkpoint(checkpoint_dirs[0])
    if not model_path:
      print('-> No model found in {}. Will try again in {} seconds'.format(checkpoint_dirs[0], eval_interval_secs))
    elif model_path == last_evaluated_model_path:
      print('-> Found already evaluated checkpoint. Will try again in {} seconds'.format(eval_interval_secs))
    else:
      ...
      metrics['Losses/Loss/total_loss'] = tot_loss

      write_metrics(metrics, global_step, summary_dir)

      # Check if new ckpt is the best one
      last_ckpt_path = model_path.split('/')
      last_ckpt_id = last_ckpt_path[-1]
      ckpt_folder_path = '/'.join(last_ckpt_path[:-1])
      best_ckpt_folder_path = os.path.join(ckpt_folder_path,'best_ckpt')

      print('-> Evaluated ckpt:{}'.format(last_ckpt_id))
      print('-> mAP:{}    loc_loss:{}    cls_loss:{}    tot_loss:{}'.format(mAP,loc_loss,cls_loss,tot_loss))

      if not os.path.isdir(best_ckpt_folder_path):
        os.mkdir(best_ckpt_folder_path)
      
      #if tot_loss <= best_eval_metrics['best_loss']: # Use this line if you want to save the ckpt with minimum tot_loss
      if mAP >= best_eval_metrics['best_mAP']: # Use this line if you want to save the ckpt with best mAP

        best_eval_metrics['best_tot_loss'] = tot_loss
        best_eval_metrics['best_mAP'] = mAP
        print('-> Found new best mAP:{} with new best loss:{}'.format(best_eval_metrics['best_mAP'],best_eval_metrics['best_tot_loss']))

        # Remove old ckpt files
        for f in os.listdir(best_ckpt_folder_path):
          if 'model.ckpt' in f:
            os.remove(os.path.join(best_ckpt_folder_path,f))

        # Copy new ckpt files to best_ckpt folder
        for f in os.listdir(ckpt_folder_path):
          if last_ckpt_id in f:
            copyfile(os.path.join(ckpt_folder_path,f),os.path.join(best_ckpt_folder_path,f))
        
        copyfile(os.path.join(ckpt_folder_path,'checkpoint'),os.path.join(best_ckpt_folder_path,'checkpoint'))
        copyfile(os.path.join(ckpt_folder_path,'pipeline.config'),os.path.join(best_ckpt_folder_path,'pipeline.config'))
        copyfile(os.path.join(ckpt_folder_path,'graph.pbtxt'),os.path.join(best_ckpt_folder_path,'graph.pbtxt'))
        copyfile(os.path.join(ckpt_folder_path,'pipeline.config'),os.path.join(best_ckpt_folder_path,'pipeline.config'))

        # Write txt summary
        with open(os.path.join(best_ckpt_folder_path,'best_ckpt.txt'), "a+") as f:
          f.write('Ckpt:{}    Best mAP:{}    Best loss:{}\n'.format(last_ckpt_id,best_eval_metrics['best_mAP'],best_eval_metrics['best_tot_loss']))

        print('-> Saved ckpt {} to {}'.format(last_ckpt_id,best_ckpt_folder_path))
