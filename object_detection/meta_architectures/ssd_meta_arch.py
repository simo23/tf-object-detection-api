...
class SSDMetaArch(model.DetectionModel):
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      detection_scores_with_background = self._score_conversion_fn(class_predictions)
      detection_scores = tf.slice(detection_scores_with_background, [0, 0, 1], [-1, -1, -1])
      additional_fields = None

      ###################################################################################################################################
      # Repeat the 2nd dimension n_classes times if you want to apply class-nms 
      #n_classes = (detection_scores.get_shape().as_list())[-1]
      #detection_boxes = tf.tile(detection_boxes, multiples=[1,1,n_classes,1])
      ###################################################################################################################################
